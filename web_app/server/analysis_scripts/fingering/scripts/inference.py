...
data_path = os.path.join('test_sequences', 'test_data.txt')
...
def evaluate():
    ...
    for i in range(5):
        ...
        fingering = np.argmax(logits, axis=-1)
        print("accuracy", np.mean(padded_fingerings[i] == fingering[0]))
        
        fingering = fingering + 1
        print("fingering", fingering)        
        
        plt.plot(padded_fingerings[i] + 1, label='truth', linewidth=2) 
