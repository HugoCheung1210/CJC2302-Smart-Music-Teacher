...
ROOT_PATH = '/Users/hugocheung/Desktop/Genre_transfer/CycleGAN-Music-Style-Transfer/ALL_Data/Cls_Jazz'
converter_path = os.path.join(ROOT_PATH, 'classical_preprocess/convert')
cleaner_path = os.path.join(ROOT_PATH, 'classical_preprocess/clean')
...
def main():
    ...
    midi_paths = get_midi_path(os.path.join(ROOT_PATH, 'classical/train'))
    midi_dict = {}
    ...
    with open(os.path.join(ROOT_PATH, 'classical_midis.json'), 'w') as outfile:
        json.dump(midi_dict, outfile)

    print("[Done] {} files out of {} have been successfully converted".format(len(midi_dict), len(midi_paths)))

    with open(os.path.join(ROOT_PATH, 'classical_midis.json')) as infile:
    ...
    with open(os.path.join(ROOT_PATH, 'classical_midis_clean.json'), 'w') as outfile:
